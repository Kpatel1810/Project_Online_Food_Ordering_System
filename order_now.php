...
$u=$_SESSION['user'];
include("connect.php");
$qu=mysql_query("select * from  registration_table where rid=$u")or die ("QF1");
$datau=mysql_fetch_array($qu);

$i=$_REQUEST['i'];
$qi=mysql_query("select * from food where F_ID=$i")or die ("QF2");
$datai=mysql_fetch_array($qi);
...
extract($_POST);
mysql_query("insert into food_orders
 (fid,rid,price,qty,total) values ('$i','$u','$pr','$qty','$tot')")or die ("QF");
